...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await db_x.set_block_message(db_x.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await db_x.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await db_x.get_pm_settings()
    await db_x.allow_user(chat_id)
