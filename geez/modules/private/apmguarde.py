...
async def pmguard(client, message):
    arg = get_arg(message)
    if not arg:
        await message.edit("**Apaan dah luh**")
        return
    if arg == "off":
        await geez.set_pm(False)
        await message.edit("**PM Guard Mati Anjay**")
    if arg == "on":
        await geez.set_pm(True)
        await message.edit("**PM Guard Aktif Anjay**")
...
async def setpmmsg(client, message):
    ...
    if arg == "default":
        ...
        await message.edit("**Pesan anu ke anu**.")
        return
    await geez.set_permit_message(f"`{arg}`")
    await message.edit("**Custom PM Guard Ke Set**")
...
add_command_help(
    ...
    [
        ...
        ["ok|a|setuju", " -> Allows a user to PM you."],
        ["no|tolak|disa", " -> Denies a user to PM you."],
    ],
)
